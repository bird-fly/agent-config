...
┌─────────────────────────────────────────────┐
│ 插件中心 (统一存储)                          │
│ C:\Users\xxx\.localAIPlugins\                │
│ └─ understand-anything → (指向仓库)          │
└─────────────────────────────────────────────┘
          │                │                │
...
```
%USERPROFILE%\.claude\
├── plugins\                    # Claude 原生插件（保留）
│   └── cache\
│       └── understand-anything\
├── understand-anything\        # Junction → .localAIPlugins
└── skills\                     # 技能（setup.ps1 同步）
    ├── understand\
    ├── understand-chat\
    └── ...
```
